...
if __name__ == "__main__":
    ...
    async def main():
        ...
        hotel_details_path = os.path.join(
            ...
            str(adults),
            str(rooms),
            f"limit_{hotel_details_limit}.csv",
        )
        ...
        try:
            ...
            print(
                f"Properties file not found at {properties_path}. Cannot perform prediction."
            )
            return
        ...
        specific_property_df = df_properties[
            df_properties["name"].str.contains(target_hotel_name, case=False, na=False)
        ]

        if specific_property_df.empty:
            print(
                f"Target hotel '{target_hotel_name}' not found in scraped properties. Using all properties for prediction."
            )
            df_properties_for_prediction = df_properties
        else:
            print(
                f"Found target hotel '{target_hotel_name}'. Predicting for this hotel."
            )
        ...
        advanced_model_filepath = get_model_filepath(
            destination,
            adults,
            rooms,
            properties_limit,
        ...
        if os.path.exists(os.path.join(advanced_model_filepath, "tf_model.keras")):
            print(
                f"Found advanced model artifacts at {advanced_model_filepath}. Attempting to use advanced model."
            )
            ...
            try:
                ...
                print(
                    f"Loaded {len(df_hotel_details)} hotel details from {hotel_details_path} for advanced model."
                )
            except FileNotFoundError:
                print(
                    f"Hotel details file not found at {hotel_details_path}. Cannot perform advanced prediction. Falling back to basic model."
                )
                df_hotel_details = None
                model_type_to_use = (
                    "basic"  # Fallback if hotel details not found for advanced
                )
        else:
            print(
                f"No advanced model artifacts found at {advanced_model_filepath}. Using basic model."
            )
